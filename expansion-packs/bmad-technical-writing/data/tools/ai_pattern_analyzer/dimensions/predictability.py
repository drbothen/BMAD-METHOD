...
class PredictabilityDimension(DimensionStrategy):
    ...
    def weight(self) -> float:
        """Return dimension weight (20% of total score - highest single dimension)."""
        return 20.0
        ...
        **kwargs
        ...
        if isinstance(prepared, list):
            ...
            for position, sample_text in samples:
                gltr_metrics = self._calculate_gltr_metrics_with_timeout(sample_text, timeout=120)
                sample_results.append(gltr_metrics)

            # Aggregate metrics from all samples
            aggregated = self._aggregate_gltr_metrics(sample_results)
    ...
    def calculate_score(self, metrics: Dict[str, Any]) -> float:
        """
        Calculate 0-100 score based on GLTR top-10 percentage.

        Lower GLTR top-10 percentage = higher score (human-like).
        Higher GLTR top-10 percentage = lower score (AI-like).

        Algorithm:
        - GLTR top-10 < 50% = 100 score (Very human-like)
        - GLTR top-10 < 60% = 75 score (Good)
        - GLTR top-10 < 70% = 50 score (Borderline)
        - GLTR top-10 >= 70% = 25 score (Strong AI signature)

        Research thresholds:
        - Human: <55% top-10 tokens
        - AI: >70% top-10 tokens
