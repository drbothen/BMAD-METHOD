...
class SentimentDimension(DimensionStrategy):
    ...
    def weight(self) -> float:
        """Return dimension weight (17% of total score)."""
        return 17.0
    ...
    def calculate_score(self, metrics: Dict[str, Any]) -> float:
        """
        Calculate 0-100 score based on sentiment variation metrics.

        Scoring logic extracted from dual_score_calculator.py.
        Higher emotional variation = higher score (human-like).
        Flat emotional tone = low score (AI-like).

        Algorithm:
        - Variance >= 0.20 = VERY HIGH (100 score)
        - Variance >= 0.15 = HIGH (75 score)
        - Variance >= 0.10 = MEDIUM (62.5 score)
        - Variance >= 0.05 = LOW (50 score)
        - Variance < 0.05 = VERY LOW (25 score)
        ...
        if variance >= 0.20:
            score = 100.0  # Strong emotional variation
        elif variance >= 0.15:
            score = 75.0  # Good variation (human-like)
        elif variance >= 0.10:
            score = 62.5  # Moderate variation
        elif variance >= 0.05:
            score = 50.0  # Limited variation (AI-like)
        else:
            score = 25.0  # Flat affect (strong AI signal)
