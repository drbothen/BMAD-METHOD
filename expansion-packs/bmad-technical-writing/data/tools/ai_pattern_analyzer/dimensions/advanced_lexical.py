...
from ai_pattern_analyzer.core.analysis_config import AnalysisConfig, DEFAULT_CONFIG, AnalysisMode
...
class AdvancedLexicalDimension(DimensionStrategy):
    ...
    def weight(self) -> float:
        """Return dimension weight (14% of total score)."""
        return 14.0
    ...
    def calculate_score(self, metrics: Dict[str, Any]) -> float:
        """
        Calculate 0-100 score based on HDD and Yule's K (primary indicators).

        Higher HDD = higher score (more diverse = human-like).
        Lower Yule's K = higher score (less repetitive = human-like).

        Algorithm:
        - HDD >0.7 AND Yule's K <50 = 100 score (Excellent diversity)
        - HDD >0.5 AND Yule's K <100 = 75 score (Good diversity)
        - HDD >0.3 AND Yule's K <200 = 50 score (Fair diversity)
        - Otherwise = 25 score (Poor diversity - AI-like)

        Research thresholds:
        - AI: HDD 0.40-0.55, Yule's K 100-150
        - Human: HDD 0.65-0.85, Yule's K 60-90
        ...
        if hdd > 0.7 and yules_k < 50:
            score = 100.0  # Excellent diversity
        elif hdd > 0.5 and yules_k < 100:
            score = 75.0  # Good diversity
        elif hdd > 0.3 and yules_k < 200:
            score = 50.0  # Fair diversity
        else:
            score = 25.0  # Poor diversity - AI-like
