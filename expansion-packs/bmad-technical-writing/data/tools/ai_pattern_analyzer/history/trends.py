...
def generate_dimension_trend_report(history: ScoreHistory, top_n: int = 5) -> str:
    ...
    if improvements:
        ...
        for i, dim in enumerate(improvements[:top_n], 1):
            ...
            output.append(f"     {dim['first']:.1f} → {dim['last']:.1f}  ({_format_score_change(dim['change'])} pts)  {indicator}  {impact}")
        output.append("")
    ...
    if plateaued:
        ...
        for dim_name in plateaued[:top_n]:
            if dim_name in last.dimensions:
                ...
                output.append(f"  - {dim_name}: {dim.score:.1f}/{dim.max_score} ({dim.percentage:.0f}%)")
        output.append("")
    ...
    if declines:
        ...
        for dim in declines[:top_n]:
            indicator = _calculate_trend_indicator(dim['change'])
            output.append(f"  - {dim['name']}: {dim['first']:.1f} → {dim['last']:.1f}  ({_format_score_change(dim['change'])} pts)  {indicator}")
        output.append("")
    else:
...
def generate_aggregate_trend_report(history: ScoreHistory) -> str:
    ...
    output.append(f"  Quality:   {first.quality_score:.1f} → {last.quality_score:.1f}  ({_format_score_change(qual_change)} pts)  {qual_trend} {qual_indicator}")
    output.append(f"  Detection: {first.detection_risk:.1f} → {last.detection_risk:.1f}  ({_format_score_change(det_change)} pts)  {det_trend} {det_indicator}")
    output.append("")
...
def generate_tier_trend_report(history: ScoreHistory) -> str:
    ...
    for tier_name, trend in tier_trends.items():
        ...
        output.append(f"  {tier_name}:  {trend['first']:.1f} → {trend['last']:.1f}  " +
...
def generate_comparison_report(history: ScoreHistory, idx1: int, idx2: int) -> str:
    ...
    output.append(f"  Quality Score                {score1.quality_score:>7.1f}        {score2.quality_score:>7.1f}      {qual_change:>+6.1f}    {_get_comparison_impact(qual_change, 'quality')}")
    output.append(f"  Detection Risk               {score1.detection_risk:>7.1f}        {score2.detection_risk:>7.1f}      {det_change:>+6.1f}    {_get_comparison_impact(-det_change, 'detection')}")
    output.append("")

    # Only compare dimensions if both are v2.0
    if score1.history_version == '2.0' and score2.history_version == '2.0' and score1.dimensions and score2.dimensions:
        # Tier comparison
        ...
        for tier_num in [1, 2, 3, 4]:
            ...
            output.append(f"  {tier_names[tier_num]:<26} {val1:>7.1f}        {val2:>7.1f}      {change:>+6.1f}    {_get_comparison_impact(change, 'tier')}")
        output.append("")
        ...
        if significant_changes:
            ...
            for dim in significant_changes:
                ...
                output.append(f"  {dim['name']:<35} {dim['val1']:>5.1f}          {dim['val2']:>5.1f}      {dim['change']:>+6.1f}    {impact} {indicator}")
            output.append("")
...
def generate_full_history_report(history: ScoreHistory) -> str:
    ...
    output.append(f"Iterations: {len(history.scores)} ({history.scores[0].timestamp[:10]} to {history.scores[-1].timestamp[:10]})")
    output.append("=" * 80)
    output.append("")
    ...
    for i, score in enumerate(history.scores, 1):
        ...
        if i > 1:
            ...
            output.append(f"Changes:       Quality {_format_score_change(qual_change)} pts, " +
    ...
    if len(history.scores) >= 3:
        ...
        output.append(f"Quality:       {qual_spark}  {qual_values[0]:.0f}→{qual_values[-1]:.0f} " +
                     f"({_format_score_change(qual_values[-1] - qual_values[0])} pts)")
        output.append(f"Detection:     {det_spark}  {det_values[0]:.0f}→{det_values[-1]:.0f} " +
        ...
        if v2_scores and len(v2_scores) >= 3:
            ...
            for dim_name, values, change in dim_changes[:5]:
                ...
                indicator = _calculate_trend_indicator(change)
                output.append(f"{dim_name:<30} {spark}  {values[0]:.1f}→{values[-1]:.1f} " +
    ...
    output.append(f"  Quality:       {first.quality_score:.1f} → {last.quality_score:.1f}  " +
                 f"({_format_score_change(qual_total)} pts, {(qual_total/first.quality_score*100) if first.quality_score > 0 else 0:+.0f}% improvement)")
    output.append(f"  Detection:     {first.detection_risk:.1f} → {last.detection_risk:.1f}  " +
    ...
    output.append(f"  Quality target (≥85):     {'✓ MET' if qual_ready else f'✗ NOT MET (need {85 - last.quality_score:+.1f} pts)'} ({last.quality_score:.1f})")
    output.append(f"  Detection target (≤30):   {'✓ MET' if det_ready else f'⚠ NOT MET ({last.detection_risk:.1f})'} ({last.detection_risk:.1f})")
    output.append("")
...
def generate_raw_metric_trends(history: ScoreHistory, metric_names: Optional[List[str]] = None) -> str:
    """
    ...
    for metric_name in metrics_to_show:
        ...
        output.append(f"  {spark}  {first_val:.2f} → {last_val:.2f}  " +
