...
function validate_link_type(link_type) {
  valid_types = ['supports', 'contradicts', 'elaborates', 'analogous_to',
                 'generalizes', 'specializes', 'influences']

  if (!valid_types.includes(link_type)) {
    throw ValidationError(`Invalid link_type: ${link_type}. Must be one of: ${valid_types.join(', ')}`)
  }
...
```javascript
function create_bidirectional_link_safe(source_path, target_path, context_forward, context_backward) {
    ...
    if (source_updated && !target_updated) {
      try {
        ...
        return {
          ...
          error: 'CRITICAL: Target update failed AND rollback failed. Manual intervention required.',
