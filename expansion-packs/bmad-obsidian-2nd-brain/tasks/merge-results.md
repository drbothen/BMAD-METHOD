...
function is_claim(sentence) {
  ...
  const has_subject_predicate = /\w+\s+(is|are|was|were|has|have|does|do|can|will|should|must)/i.test(sentence)
  ...
  if (numbers_a.length > 0 && numbers_b.length > 0) {
    ...
    for (const num_a of numbers_a) {
      for (const num_b of numbers_b) {
        ...
        if (avg > 0 && diff / avg > 0.5) {  // >50% difference
...
function are_opposite_actions(verb_a, verb_b) {
  ...
  for (const [opp1, opp2] of opposite_pairs) {
    if ((verb_a.includes(opp1) && verb_b.includes(opp2)) ||
...
function extract_adjectives(text) {
  ...
  const adjective_pattern = /\b(good|bad|effective|useful|simple|complex|fast|slow|easy|difficult|important|trivial)\b/gi
...
function extract_action_verbs(text) {
  const verb_pattern = /\b(improves?|worsens?|increases?|decreases?|enhances?|diminishes?|supports?|contradicts?|helps?|hinders?)\b/gi
