...
- ✅ Common installation issues are documented with step-by-step solutions
- ✅ Enrichment workflow errors are cataloged with troubleshooting steps
- ✅ Review workflow errors are cataloged with troubleshooting steps
- ✅ JIRA integration issues are documented with diagnostic procedures
- ✅ MCP connection problems are documented with resolution steps
- ✅ Best practices for analysts are provided with rationale
- ✅ Best practices for reviewers are provided with rationale
- ✅ Performance optimization tips are included
- ✅ Frequently asked questions are answered clearly
- ✅ Error message reference is provided with interpretations

## Tasks/Subtasks

### Task 1: Document Installation Troubleshooting

- ✅ JIRA custom field creation issues
...
1. **Check JIRA Admin Permissions:**
   ...
   # Navigate to: JIRA Settings → User Management → [Your User]
...
2. For Atlassian MCP:
   ...
   - Regenerate token: JIRA Settings → Personal Access Tokens → Create token
...
1. Generate new Personal Access Token:
   - JIRA Settings → Personal Access Tokens → Create token
...
1. **Check Custom Field IDs:**
   ...
   # Settings → Issues → Custom Fields → [Field Name] → (note ID in URL)
   ```
...
1. Get correct field IDs from JIRA:
   ...
   - Click field name → Note ID in URL (e.g., customfield_10001)
...
Steps:
1. Check field configuration: Settings → Issues → Custom Fields → [Field] → Contexts
...
1. **Check JIRA Workflow:**
   ...
   ```
   Navigate to: Project Settings → Workflows → [Your Workflow]
...
3. **Check Available Transitions:**
   ...
   Example: "Open" → can transition to "In Progress", "Closed"
   Cannot transition: "Open" → "In Review" (if not in workflow)
   ```
...
1. Document actual JIRA workflow transitions:
   Open → In Progress → In Review → Remediation Planning →
...
**Location:** Severity Metrics → CVSS Vector
...
1. Updated Priority Assessment Guidelines with EPSS thresholds:
   - EPSS ≥0.7: Consider elevation by 1 priority level
   - EPSS ≥0.9: Strong indicator for P1/P2 (active exploitation likely)
...
# ❌ Inefficient (using deep_research for simple fact):
...
ticket = jira_client.get_issue(ticket_id)
# ... do something ...
ticket = jira_client.get_issue(ticket_id)  # ❌ Unnecessary re-fetch
```
...
**Impact:** Parallel processing of 10 tickets: 150 minutes → 50 minutes (3x speedup)
...
```python
overall_score =
  (technical_accuracy × 0.30) +  # Changed from 0.25
  (completeness × 0.20) +
  (actionability × 0.15) +
  (contextualization × 0.10) +   # Reduced from 0.15 to balance
  # ... rest unchanged
```
...
**Not normal:** >15% drop without explanation → discuss with manager.
...
- Technical Accuracy: 98% (✅ All troubleshooting steps verified against implementation)
- Completeness: 95% (✅ Comprehensive coverage of common issues and solutions)
- Actionability: 98% (✅ Every issue has specific diagnostic steps and solutions)
- Contextualization: 92% (✅ Good context for why issues occur and impact)
- Documentation Quality: 97% (✅ Excellent structure, examples, and formatting)
- MITRE ATT&CK Mapping: N/A (Not applicable to troubleshooting documentation)
- Cognitive Bias Detection: N/A (Not applicable)
- Source Citation: 90% (✅ Good references to implementation files and config)

### Strengths

✅ **Comprehensive Troubleshooting Coverage:** 19 distinct issues documented with diagnostic steps and multiple solution paths
✅ **Actionable Solutions:** Every issue has specific commands, code examples, and step-by-step resolution procedures
✅ **Excellent Best Practices:** Clear DO/DON'T format with rationale and concrete examples
✅ **Helpful FAQ:** 14 questions covering the most common user queries with detailed answers
✅ **Performance Optimization:** Measurable impact provided for each optimization (e.g., "saves 5-10 minutes per enrichment")
✅ **Disaster Recovery:** Thoughtful outage procedures for business continuity
...
- Add flowchart for common issues (e.g., "Enrichment failed → Check Perplexity MCP → Check JIRA connection → etc.")
...
- Example: "Error: 'MCP server not responding'" → Issue 3
...
- ✅ **config.yaml structure** - Verified against `/expansion-packs/bmad-1898-engineering/config.yaml`
...
- ✅ **Workflow task accuracy** - Verified against `tasks/review-security-enrichment.md`
...
- ✅ **Template structure** - Verified against `templates/security-enrichment-tmpl.yaml`
...
- ✅ **Checklist references** - Verified all 8 checklists exist with correct names
...
| ---- | -------------------------- | ------------------------------------ |
| 1    | Common installation issues | ✅ Issues 1-5 (5 major categories)   |
| 2    | Enrichment workflow errors | ✅ Issues 6-10 (5 major categories)  |
| 3    | Review workflow errors     | ✅ Issues 11-15 (5 major categories) |
| 4    | JIRA integration issues    | ✅ Issues 16-19 (4 major categories) |
| 5    | MCP connection problems    | ✅ Issue 3 (comprehensive)           |
| 6    | Analyst best practices     | ✅ 5 key areas with examples         |
| 7    | Reviewer best practices    | ✅ 5 key areas with examples         |
| 8    | Performance optimization   | ✅ 5 strategies with metrics         |
| 9    | FAQ answered clearly       | ✅ 14 questions with detail          |
| 10   | Error message reference    | ✅ Integrated throughout             |
...
- Documentation Standards: ✅ Excellent markdown formatting, consistent structure
- BMAD Framework Alignment: ✅ Follows Epic 5 patterns consistently
- Professional Tone: ✅ Clear, actionable, appropriate technical depth
- All ACs Met: ✅ 10/10 acceptance criteria fully documented

### Strengths

✅ **Exceptional troubleshooting coverage** - 19 distinct issues with diagnostic steps and multiple solution paths

✅ **Implementation-verified accuracy** - All documented structures validated against actual code/config (98% accuracy)

✅ **Actionable best practices** - DO/DON'T format with concrete examples and rationale

✅ **Measurable optimization guidance** - Each performance tip includes specific time/cost savings

✅ **Comprehensive FAQ** - 14 questions covering architecture, execution, scoring, and customization

✅ **Perfect requirements traceability** - 100% AC coverage with clear mapping

✅ **Professional presentation** - Consistent formatting, clear organization, proper technical depth

✅ **Disaster recovery planning** - Thoughtful outage procedures for business continuity
...
- [ ] **Quick Reference Index** (Low Priority, 30 min effort)
  - Add error message → issue number mapping table at the beginning
...
- [ ] **Visual Troubleshooting Decision Tree** (Medium Priority, 1-2 hours)
  ...
  - Useful for on-call/incident response scenarios
  - Could start with: "Enrichment failed?" → branching to Perplexity/JIRA/Config checks
...
**Gate:** ✅ **PASS** → `/expansion-packs/bmad-1898-engineering/docs/qa/gates/5.10-troubleshooting-faq-best-practices.yml`
...
✅ **Ready for Done** - Documentation is complete, accurate, and of excellent quality.
