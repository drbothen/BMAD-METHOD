...
- **Standalone report:** `fact-verification-report-tmpl.yaml` → markdown output
...
2. *fact-check (OPTIONAL) → Verify critical claims (Story 2.5) ← THIS STORY
...
- Unknown: ⚠️ Cannot verify (source unavailable or conflicting)
...
- **No Data Available:** Note as "⚠️ Unable to verify - information not yet available (new CVE)"
...
1. **CVSS Verification:**
   ...
   Verification: ❌ MISMATCH - Critical Discrepancy
...
3. **KEV Status Verification:**
   ...
   Verification: ❌ MISMATCH - Significant Discrepancy
...
5. **Exploit Status Verification:**
   ...
   Verification: ⚠️ UNABLE TO INDEPENDENTLY CONFIRM
...
### 🔴 Critical Discrepancy #1: CVSS Score Mismatch
...
### 🟡 Significant Discrepancy #2: KEV Status Incorrect
...
- ✅ EPSS Score: 0.85 (Verified: FIRST.org)
- ✅ Affected Versions: Apache Struts 2.0.0-2.5.32 (Verified: Apache Advisory)
- ✅ Patched Version: 2.5.33+ (Verified: Apache Advisory)

---

## Unable to Verify (⚠️)

- ⚠️ Exploit Status: "Active Exploitation" - Unable to confirm independently
```
...
- Note in review: "⚠️ Fact verification skipped (Perplexity unavailable)"
...
1. **Perfect Accuracy:** Enrichment with all correct claims → 100% accuracy score
   - CVSS matches NVD
   - EPSS matches FIRST
...
6. **Perplexity MCP Unavailable:** MCP server not configured or unreachable
   ...
   - Report note: "⚠️ Fact verification skipped (Perplexity MCP unavailable)"
...
- **BMAD Task Pattern:** ✅ PASS - All required sections present (Purpose, When to Use, Inputs, Process, Error Handling, Security Considerations)
- **BMAD Test Pattern:** ✅ PASS - Markdown-based test documentation with mock strategies, clear test cases, and pass criteria
- **Source Tree Structure:** ✅ PASS - Files in correct locations (tasks/, tests/tasks/)
- **Story File Structure:** ✅ PASS - All required sections present with proper format
- **All ACs Met:** ✅ PASS - All 5 acceptance criteria fully covered with clear implementation and test mappings
...
- ✅ Mock strategy clearly defined for deterministic testing
- ✅ Expected behaviors and pass criteria explicit for each test
- ✅ Edge cases well-covered (new CVEs, conflicting sources, MCP unavailable)
- ✅ Proper test level separation (unit/integration/security)
...
- ✅ CVE ID regex validation (^CVE-\d{4}-\d{4,}$) - task:38-62
- ✅ Ticket ID sanitization to prevent injection - task:42-44
- ✅ File path validation (no ../ traversal) - task:45
- ✅ Query parameter sanitization - task:46-54

**API Security:**

- ✅ HTTPS-only for all external API calls - task:515-519
- ✅ Request timeouts (30s per query, 5min total) - task:521-524
- ✅ No credential exposure in errors/logs - task:526-530
- ✅ Response validation before parsing - task:531-537

**Rate Limiting & DoS Prevention:**

- ✅ 1 query/second maximum rate - task:101-106, 538-543
- ✅ Max 10 queries (critical mode), 20 (comprehensive mode)
- ✅ User warning if comprehensive mode exceeds limits
- ✅ Total execution timeout (5 minutes)

**Data Handling:**

- ✅ XSS prevention (sanitize external data in markdown) - task:536
- ✅ Error message sanitization (no internal paths) - task:537
- ✅ Audit logging for security trail - task:545-550

**Security Tests:**

- ✅ TC-FACT-SEC-001: Input sanitization validation
- ✅ TC-FACT-SEC-002: Error message sanitization
- ✅ TC-FACT-008: Security validation failure handling
...
- ✅ Rate limiting prevents API abuse (1 query/sec)
- ✅ Query limits prevent excessive delays (10 critical, 20 comprehensive)
- ✅ Timeouts prevent hanging operations
- ✅ Estimated execution time documented (10-15s critical, 30-60s comprehensive)

**Scalability:**

- ✅ Selective verification (critical vs comprehensive modes)
- ✅ Parallel potential not utilized (sequential by design for rate limiting)
...
**Security:** ✅ PASS (Outstanding - comprehensive security design)
**Performance:** ✅ PASS (Good - appropriate for use case with rate limiting)
**Reliability:** ✅ PASS (Excellent - graceful degradation, retry logic, fail-safe design)
**Maintainability:** ✅ PASS (Excellent - clear documentation, follows patterns, well-structured)
...
**Gate:** PASS → docs/qa/gates/2.5-fact-verification.yml
