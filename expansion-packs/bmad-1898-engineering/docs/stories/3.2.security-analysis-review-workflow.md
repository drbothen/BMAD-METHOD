...
1. Workflow includes all 7 stages: Preparation → Evaluation → Gap Identification → Bias Detection → (Optional) Fact Verification → Documentation → Feedback Loop
...
│   └── security-analysis-review-workflow.yaml  # This story creates this file
├── tasks/                            # Workflow orchestration tasks
│   ├── enrich-security-ticket.md    # Story 3.1 enrichment orchestration
│   └── review-security-enrichment.md # This story creates this file
├── checklists/                       # Quality dimension checklists (Story 2.2)
...
│   └── security-review-report-tmpl.yaml # Story 2.6 review report template
├── metrics/                          # Workflow performance metrics (to be created)
├── artifacts/                        # Review document storage (to be created)
...
12. **Next Steps**
    - If Critical Issues: "Needs Revision" → reassign to analyst
...
✅ Stage 1: Preparation (completed in 2m 15s)
   → Enrichment extracted from JIRA-1234
   → 12 factual claims identified

✅ Stage 2: Systematic Evaluation (completed in 6m 30s)
   → All 8 checklists executed
   → Overall Quality Score: 78% (Good)

✅ Stage 3: Gap Identification (completed in 3m 45s)
   → 0 Critical gaps
   → 3 Significant gaps
   → 5 Minor improvements

✅ Stage 4: Cognitive Bias Detection (completed in 2m 20s)
   → 1 bias detected: Confirmation bias

⏭️  Stage 5: Fact Verification (skipped - Perplexity MCP unavailable)

✅ Stage 6: Review Documentation (completed in 2m 10s)
   → Review report generated (12 sections)

✅ Stage 7: Feedback Loop (completed in 45s)
   → Review posted to JIRA-1234
   → Ticket assigned to analyst

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
✅ Review Complete | Total: 17m 45s
```
...
**Sequencing:**
1. Story 3.1 creates enrichment → enrichment posted to JIRA
2. Story 3.2 reviews enrichment → review posted to JIRA
3. If Critical Issues found → Story 3.1 re-enrichment may be required
...
- Future: Story 3.3 (Lifecycle Workflow) will orchestrate both 3.1 → 3.4 → 3.2 sequence
...
4. **Enrichment with critical errors**: Critical issues identified, ticket status → "Needs Revision"
5. **Optional fact verification**: Test with and without Perplexity MCP available
6. **MCP unavailable scenarios**:
   - Atlassian MCP unavailable → workflow halts with clear error message
   - Perplexity MCP unavailable → fact verification skipped, documented in report
...
11. **Feedback loop automation** - Status change rules (Critical → Needs Revision, Significant → In Review, Minor → Approved), analyst assignment, file storage (artifacts/reviews/, metrics/)
...
- Coding Standards: ✓ (N/A - natural language workflow definition)
...
- Testing Strategy: ✓ Comprehensive integration test suite with 10 test cases
- All ACs Met: ✓ All 4 acceptance criteria fully implemented

### Requirements Traceability

**AC1: Workflow includes all 7 stages**
- ✓ **Implementation:** workflows/security-analysis-review-workflow.yaml:13-261 defines all 7 stages
- ✓ **Validation:** Each stage has inputs, outputs, success_criteria, error_handling
- ✓ **Test Coverage:** TC-001 validates sequential execution of all 7 stages
...
- ✓ **Validation:** Time tracking per stage with total execution validation
- ✓ **Test Coverage:** TC-010 Performance Validation verifies 90th percentile ≤ 20 minutes
...
- ✓ **Validation:** Template-driven report generation with all sections
- ✓ **Test Coverage:** TC-001, TC-002, TC-003 all verify JIRA comment with required sections
...
- ✓ **Implementation:**
  ...
  - Stage 7 status_change_rules (lines 233-237) maps critical issues → "Needs Revision"
  - JIRA assignment operation (lines 585-592) notifies analyst
- ✓ **Validation:** Deterministic status change based on critical issue detection
- ✓ **Test Coverage:** TC-003 validates status change and analyst notification
...
- TC-010 validates 90th percentile ≤ 20 minutes
...
**Gate:** PASS → docs/qa/gates/3.2-security-analysis-review-workflow.yml
