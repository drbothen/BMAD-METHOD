...
- [ ] Only re-alert if severity level increases (WARNING → VIOLATED)
...
- [ ] Orchestrate: query JIRA → calculate SLA → send alerts → log results
...
<html>
  ...
  <body>
    <div class="alert-box">
      ...
      <p><a href="{{jira_url}}">Open Ticket in JIRA →</a></p>
    </div>
  </body>
</html>
```
        ...
        if len(alert) >= 3:
            ...
            if alerted_ticket_id == ticket_id:
                if alerted_severity == 'VIOLATED':
                    ...
                    # Escalation: WARNING → VIOLATED, send new alert
                    return True
...
- Story 3.1 (Enrichment Workflow) - Enrichment date field populated ✅
- Story 3.3 (Lifecycle Workflow) - Ticket status transitions defined ✅
- Story 5.9 (Metrics Documentation) - SLA definitions documented ✅
