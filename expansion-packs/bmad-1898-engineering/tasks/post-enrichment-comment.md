...
- Invalid ticket ID: "❌ Invalid ticket ID format. Expected format: PROJECT-123"
- File not found: "❌ Enrichment file not found: {enrichment_file_path}"
- File empty: "❌ Enrichment file is empty. Cannot post empty comment."
- File too large: "⚠️ Enrichment file is {size}MB. JIRA may reject large comments. Proceeding with caution..."
...
- "❌ Cannot read enrichment file: {error_message}"
...
1. **Add header section with visual hierarchy:**
   ...
   **Version Source:** Read version from `expansion-packs/bmad-1898-engineering/config.yaml → version` field
...
2. **Add emoji indicators to sections:**
   ...
   - Priority Assessment: Priority emojis (🔴 P1, 🟠 P2, 🟡 P3, 🔵 P4, ⚪ P5)
   - References: `📚`
...
6. **Add enrichment footer:**
   ...
   🤖 _Generated by BMAD-1898 Security Analyst Agent v{version}_
...
- If > 100,000 characters: Display warning "⚠️ Comment is {length} characters. May need truncation if JIRA rejects."
...
- "✅ Posted enrichment comment to {ticket_id}"
...
- Display: "❌ Cannot add comment to {ticket_id}. Check JIRA permissions."
...
- Display: "❌ Comment rejected by JIRA (too large: {size} characters)"
...
- If truncated version posts successfully: "✅ Posted truncated enrichment comment. Full enrichment saved to: {local_file_path}"
...
- Display: "💾 Enrichment saved to {local_file_path}. Retry posting when connection is restored."
- HALT

**Rate Limit Exceeded (429):**

- Display: "⚠️ JIRA API rate limit exceeded. Retrying with exponential backoff..."
- Retry Strategy:
...
- Display retry status: "⏳ Retry attempt {n}/3 - waiting {seconds}s..."
- If all retries fail: "❌ Rate limit persists after 3 attempts. Enrichment saved to {local_file_path}"
- HALT

**Invalid Ticket ID (404):**

- Display: "❌ Ticket {ticket_id} not found. Verify ticket ID and try again."
- HALT

**Authentication Failure (401):**

- Display: "❌ JIRA authentication failed. Check Atlassian MCP configuration."
- Action: "Verify MCP server credentials are valid and not expired."
- HALT

**Other Errors:**

- Display: "❌ Error posting comment: {error_type} - {error_message}"
- Action: "Saving enrichment locally for manual posting..."
- Save enrichment to: `.ai/enrichment-{cve_id}-{timestamp}.md`
- Display: "💾 Enrichment saved to {local_file_path}. Please post manually or retry."
- HALT
...
4. Display: "✅ Verified: Comment successfully posted to {ticket_id}"

**If verification fails:**

- Display: "⚠️ Cannot verify comment posting. Check JIRA manually: https://{jira_domain}/browse/{ticket_id}"
...
```
✅ Posted enrichment comment to AOD-1234
